...
int main(int argc, char *argv[])
{
    ...
    static unsigned int blockSequenceCheck = 0;
    ...
    while(1) 
    {
        ...
        if( ( iRecResult = IpcReceive(printerQueueId, msgBuf, sizeof msgBuf, &byteRead) ) == 0 )  // Wait for response message       
        {
            ...
            case  IPC_PRINTER_JSON:
                std::cout<<"IPC_PRINTER_JSON--:"<< IPC_PRINTER_JSON << std::endl;
                if(((IPC_PayloadMulti_t *)recvMsg)->blockSequenceNumber <= ((IPC_PayloadMulti_t *)recvMsg)->numberOfBlocks &&
                        ((IPC_PayloadMulti_t *)recvMsg)->blockSequenceNumber > 0
                  )
                {
                    blockSequenceCheck++;
                    if(((IPC_PayloadMulti_t *)recvMsg)->blockSequenceNumber == blockSequenceCheck)
                        jsonData.append(((IPC_PayloadMulti_t *)recvMsg)->data);   
                    else
                        std::cout <<"Wrong IPC MSG Sequence number"<<std::endl;
                    if(((IPC_PayloadMulti_t *)recvMsg)->blockSequenceNumber == ((IPC_PayloadMulti_t *)recvMsg)->numberOfBlocks)
                    {       
                        blockSequenceCheck = 0;
                        mykiPrinter.processIPCJson(jsonData);
                        jsonData.clear();
                    }
                }
                else
                    std::cout <<"Received IPC Block sequence number is out of the specified range" <<std::endl;
                break;
            }
        }
        else
